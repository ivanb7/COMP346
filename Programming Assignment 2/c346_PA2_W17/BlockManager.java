...
public class BlockManager
{
	 ...
	 */
	// -9 is used so that 10 releases are needed before starting phase II
	...
	static class AcquireBlock extends BaseThread
	{
		...
		public void run()
		{
			...
			//release S1 at the end of phase I
			s1.V();
			//Acquire a permit for S1 (waits until all the phase I finish)
			s1.P();
			//release S1 once phase II is going to start
			s1.V();
			
			//Acquire a permit for S2
			s2.P();
			
			//Will go in while loop if it's not the thread's turn
			...
			phase2();
			//release S2 when phase II is complete
			s2.V();
	...
	static class ReleaseBlock extends BaseThread
	{
		...
		public void run()
		{
			...
			//release S2 once phase II is complete
			s2.V();
